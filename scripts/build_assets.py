...
RAW_DATA_PATH = os.path.join(PROJECT_ROOT, 'raw_data', 'movies_metadata.csv')

print("Starting one-time preprocessing...")

# 1. Read from raw_data
try:
    df = pd.read_csv(RAW_DATA_PATH, low_memory=False)
except FileNotFoundError:
    # Fallback if user didn't put it in raw_data
    df = pd.read_csv(os.path.join(PROJECT_ROOT, 'movies_metadata.csv'), low_memory=False)
...
with open(os.path.join(DATA_OUTPUT_DIR, 'language_columns.json'), 'w', encoding='utf-8') as f:
...
with open(os.path.join(DATA_OUTPUT_DIR, 'genre_columns.json'), 'w', encoding='utf-8') as f:
